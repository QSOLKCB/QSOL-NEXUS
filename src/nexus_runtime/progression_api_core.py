...
from .life_paths import apply_life_paths_choice, inspect_life_paths, life_paths_catalog, new_life_paths
from .modes import get_mode
from .progression import (
    ACTIVITY_CATALOG,
    MAX_ACTIVITY_PROMPT_CHARS,
    MAX_SOURCE_REFS,
...
_PROGRESSION_OPERATIONS = frozenset(
    {
        "progression.policy", "progression.activities", "progression.commission.create",
        "progression.commission.inspect", "progression.act", "progression.play.record",
        "progression.portfolio", "life.paths.catalog", "life.paths.new",
        "life.paths.inspect", "life.paths.act",
    }
)
_PROGRESSION_MUTATIONS = frozenset(
    {"progression.commission.create", "progression.act", "progression.play.record", "life.paths.new", "life.paths.act"}
)
...
class ProgressionNexusAPI(WorldContinuityNexusAPI):
    ...
    @staticmethod
    def _activity_sources(raw: Any) -> list[str]:
    ...
    def _handle_progression_operation(self, request: dict[str, Any], request_id: str | None) -> dict[str, Any]:
        ...
        try:
            if operation == "progression.policy":
                ...
                self._require_exact_fields(request, operation, {"title", "activity_id", "brief", "source_refs", "assignee_id"})
                ...
                obj = self._run_real_mutation(lambda: self.progression.create_commission(title=title, activity_id=activity_id, brief=brief, source_refs=source_refs, assignee_id=assignee_id))
                ...
                response = self.progression.portfolio(actor_id=self._require_str(request, "actor_id"), model_id=self._require_str(request, "model_id"))
            elif operation == "progression.act":
                self._require_exact_fields(request, operation, {"member", "activity_id", "prompt", "source_refs", "commission_ref", "mode"})
                activity_id = self._require_str(request, "activity_id")
                ...
                if activity_id in {"play_monopoly", "play_life_paths"}:
                    raise ProgressionError("progression_play_requires_game_ref", "play activities must use progression.play.record with an authoritative game state")
                raw_prompt = self._require_str(request, "prompt")
                if len(raw_prompt) > MAX_ACTIVITY_PROMPT_CHARS:
                ...
                commission = None if commission_ref is None else self.progression.inspect_commission(commission_ref)
                if commission is not None and commission.payload["activity_id"] != activity_id:
                    raise ProgressionError("progression_commission_mismatch", "commission activity does not match the requested activity")
                actor = self._activity_actor(request.get("member"))
                if commission is not None:
                    if commission.payload.get("assignee_id") not in {None, actor.member.member_id}:
                ...
                activity_instruction = (
                    ...
                    + "\n\nNEXUS non-voting activity: " + activity_policy["instruction"]
                    + "\nYour contribution may become an immutable portfolio artifact. It does not create evidence authority, citizenship, tool authority, an extra Council seat, or vote weight."
                )
                if commission is not None:
                    activity_instruction += (
                        "\n\nNEXUS commission binding:\nTitle: " + commission.payload["title"]
                        + "\nBrief: " + commission.payload["brief"]
                        + "\nComplete this stored brief; do not substitute the generic operator wording for the commission itself."
                    )
                text = actor.direct_message(clean_prompt.text, mode_id=mode.mode_id, mode_instruction=activity_instruction, geometry_region_id=mode.region_id, evidence_context=evidence_context)
                clean_output = self.scrubber.scrub(text)
                try:
                    self.stenographer.observe_text(
                        "progression.activity", actor, clean_output.text,
                        stimulus={"activity_id": activity_id, "prompt": clean_prompt.text, "source_refs": list(source_refs), "commission_ref": commission_ref, "mode_id": mode.mode_id},
                        mode_id=mode.mode_id, geometry_region_id=mode.region_id, attempt="activity",
                    )
                except StenographerError as exc:
                    self.stenographer.mark_gap(exc.code)
                except Exception:
                    self.stenographer.mark_gap("observer_internal_error")
                recorded = self._run_real_mutation(lambda: self.progression.record_activity(actor_id=actor.member.member_id, model_id=actor.member.model_id, activity_id=activity_id, prompt=clean_prompt.text, output=clean_output.text, source_refs=list(source_refs), commission_ref=commission_ref))
                response = {"status": "ok", **recorded, "activity_id": activity_id, "member_id": actor.member.member_id, "model_id": actor.member.model_id, "mode_id": mode.mode_id, "secret_scrub": {"prompt_changed": clean_prompt.changed, "output_changed": clean_output.changed}, "authority_effect": "none"}
                ...
                activity_id = {"monopoly": "play_monopoly", "life_paths": "play_life_paths"}.get(game_kind)
                if activity_id is None:
                    raise ProgressionError("progression_invalid_play", "game_kind must be monopoly or life_paths")
                actor = self._activity_actor(request.get("member"))
                recorded = self._run_real_mutation(lambda: self.progression.record_play(actor_id=actor.member.member_id, model_id=actor.member.model_id, activity_id=activity_id, game_ref=game_ref, game_kind=game_kind))
                response = {"status": "ok", **recorded, "game_kind": game_kind, "authority_effect": "none"}
                ...
                for player in [*players, *human_players]:
                    if self.scrubber.scrub(player).changed:
                        raise ProgressionError("progression_secret_rejected", "Life Paths player ids must not contain credential-shaped material")
                clean_seed = self.scrubber.scrub(seed)
                state = self._run_real_mutation(lambda: new_life_paths(self.world, seed=clean_seed.text, players=players, human_players=human_players))
                response = {"status": "ok", "game_ref": state.object_id, "game": state.payload, "secret_scrub": {"seed_changed": clean_seed.changed}}
                ...
                state = self._run_real_mutation(lambda: apply_life_paths_choice(self.world, game_ref, player_id=player_id, choice_id=choice_id))
                response = {"status": "ok", "game_ref": state.object_id, "game": state.payload}
    ...
    def handle(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if operation == "world.create" and isinstance(request, dict):
            ...
            if isinstance(object_type, str) and (object_type in PROGRESSION_RESERVED_OBJECT_TYPES or object_type in _PROGRESSION_PLAY_OBJECT_TYPES):
                return self._error(safe_request_id, "invalid_request", "reserved progression/game objects require validated runtime operations")

        response = super().handle(request)
        if operation == "system.health" and response.get("status") == "ok":
            return {**response, "ai_progression": {"status": "ok", "policy": progression_policy_snapshot(), "life_paths": {"schema": life_paths_catalog()["schema"], "original_nexus_game": True, "authority_effect": "none"}}}
        if operation == "system.operations" and response.get("status") == "ok":
            operations = list(response.get("operations", [])); operations.extend(sorted(_PROGRESSION_OPERATIONS))
